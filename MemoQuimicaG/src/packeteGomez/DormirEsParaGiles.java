...
public class DormirEsParaGiles {
    public static int[] Varianza() {
        int[] almacen = new int[24];//con String fallan las operaciones
        int contador = 0;

        while (contador < 24) {//las variables aleatorias se ligan al contador, por lo que se tiene registro de ellas gracias al numero del contador, osea lo que paso cuando se ejecuto el while

            Random elRandom = new Random();
            int variableAleatoria = elRandom.nextInt(12) + 1;//se generan los 5 pares, en ordenes aleatorios
